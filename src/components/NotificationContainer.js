...
const NotificationContainer = () => {
  ...
  // Handle animation of notifications entering and exiting
  useEffect(() => {
    setAnimatedNotifications(notifications);
  }, [notifications]);
  ...
  if (animatedNotifications.length === 0) {
    return null;
  }
  ...
  return (
    <div className="fixed top-4 right-4 z-50 space-y-3 w-72">
      {animatedNotifications.map((notification) => (
        <div
          key={notification.id}
          className={`p-3 rounded shadow-md border transition-all duration-500 animate-slideIn transform hover:scale-105 ${getNotificationStyles(notification.type)}`}
        >
